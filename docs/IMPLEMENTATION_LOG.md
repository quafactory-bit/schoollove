...
- Created twelve original illustrations: seven contextual scenes and five school stages. Added a six-second hero animation and 7.5-second siege edit; a clean 27-second siege version remains in local review artifacts. Original siege clips already precede caption/brand overlays. Asset provenance is recorded in `public/images/scenes/README.md`.
- Home, search, login, onboarding, owner school cards, share preview, welcome guide and empty connections now use contextual scenes. School thresholds remain 1/2/4/7/10. Connection detail, real rank/XP authority, owner/public projections, access gates and all API/DB logic are unchanged.
- Targeted rendering/capability tests: 4 files / 54 PASS. TypeScript PASS. Full suite final run with two workers: 206 files PASS, 3 existing skipped files; 1,708 tests PASS, 4 existing skips. Updated obsolete AVIF/SVG/character-overlay assertions to new distinct-school artwork while retaining all threshold, privacy and ranking-link assertions. Initial full run had one existing crypto-boundary test failure; that unchanged file passed all 12 tests in isolation and the complete final run passed.
- `node scripts/growth-ux/scene-review.mjs`: 34 PC/mobile captures across 17 screen/state variants, no horizontal overflow/broken visible images/browser errors. Playback, pause/resume, offscreen pause, reduced-motion and video-error poster checks PASS. Reframed 10 modal captures to viewport for gallery review.
- `node scripts/welcome-guide/check.mjs`: four steps at 320/390/1280, focus/escape/replay, owner progress, closed/emergency and storage failure PASS; writes 0. `node scripts/welcome-guide/connection-copy-check.mjs`: off/private/shared × 320/390/1280 PASS; no unauthorized Instagram/message requests, writes or overflow.
- React review: stable refs, effects clean up observers/listeners, no new network API or persistence. Videos mount only when visible and motion is permitted; optional failure preserves poster and working page controls. Static illustrations use responsive Next Image. Public assets total approximately 3.2 MB; hero video 475 KB and siege banner 1.37 MB. Preview server binds loopback and uses synthetic API data only.
- Build result and final review status are recorded in `docs/verification/2026-09-15-contextual-scenes.md`. Real signup, live permissions, external Instagram DM, physical devices and Production were not exercised. Local screenshots/media: `.local/growth-ux/scene-review/`; gallery: `http://127.0.0.1:3117/visual-review`.
...
- PR #68 slug-contract review classified the repository authority as `SLUG_CONTRACT_NOT_PROVEN`: `schools.slug` is unique text without a character CHECK, no canonical validator exists, and the read-only Preview catalog contains no representative school rows. The first-value helper therefore removed its unsupported ASCII-only filter, preserves a nonempty DB slug through single-segment encoding, and still rejects slash/backslash and dot traversal segments without guessing from `school_name`. Review validation passed focused Vitest 2 files / 24 tests, TypeScript with incremental cache disabled, and the disposable Google-bound Playwright matrix 20/20 across desktop and 360/390/412 (`workers=1`, `retries=0`).
...
## 2026-08-23 — PHASE 10P Preview Google activation verified (PREVIEW / evidence frozen)

- Verified the Preview deployment `dpl_5xoV1NxpdyBS1wyFbk7TJC8F1CDy` is READY at commit `58ac7035b916ddf5d85159e5e5ecb422bab610cc`. Readiness and controlled open remain recorded exactly once; launch is `open` with all three flags true and no emergency stop.
- Preserved the Google progression: first login under `closed` became recovery-verified and Auth-principal-bound; one bound-provisional reauthentication completed without recovery; an approved short controlled Vercel Authentication OFF window under Standard Protection allowed one final Google flow after discovery/JWKS had initially been blocked; `/account` was reached and Require Log In was restored immediately.
- Final coarse evidence is attempts/transactions/legs `9/9/9`, broker codes `4` (`expired=1`, `consumed=3`), recovery `3/3` with delta `0/0`, private accounts/identities `1/1`, Auth users/identities/email identities `1/1/0`, active accounts/identities `1/1`, and duplicate mappings `0`. The existing Google account and identity are active, their activation timestamps are equal, and terminal raw-context checks are `0`.
- An unauthenticated post-flow check found both Preview discovery and JWKS behind Vercel SSO protection redirects rather than public `200`. No bypass/share credential was used or retained.
- The Google path is frozen. The pre-Kakao audit found no Kakao attempt and no provider call. Independent value-redacted Preview Supabase read-only evidence verifies the Kakao Custom OIDC tuple; Kakao Developers console state remains an operator-confirmation prerequisite before any separate Kakao E2E approval.
- Production, code, migrations, Vercel configuration, Supabase/Auth/launch state, provider configuration, email, Recovery Resend, OTP, and secrets/PII were not changed. Detailed redacted evidence: `docs/decisions/2026-08-23-preview-google-activation-verification.md`.

## 2026-08-24 — PHASE 10Q Google-only Preview auth consolidation (LOCAL / Draft)

- Local Draft code narrows the future Preview path to fixed `custom:schoollove-google` initiation, with Google as the only visible user-login control. Supabase email OTP routes and Kakao/Naver callback routes are dark 404 surfaces.
- The local Preview broker loader requires Google and shared security/recovery configuration only; Kakao/Naver credentials and downstream clients are not runtime prerequisites or authorization authorities.
- SchoolLove custom recovery email and its eight-digit OTP contract remain unchanged. No external provider, Supabase Auth configuration, Vercel environment, database, launch, or Production mutation is included.
- Final local evidence: focused Vitest `9 files / 67 tests`, full Vitest `1,220 passed / 4 skipped / 0 failed`, TypeScript, and a localhost-placeholder Next.js build (`59` static pages) passed. The Google-only Playwright contract passed once in Chromium `1440×900` and once each at mobile `360×800`, `390×844`, and `412×915`, with `workers=1`, `retries=0`, and all non-local browser requests blocked before dispatch.
- Preview durable-state read-only evidence remains unchanged: attempts/transactions/legs `10/10/10`; broker codes `4` (`expired=1`, `consumed=3`); recovery verifications/deliveries `4/4`; private accounts/identities and active accounts/identities `1/1` each; Auth users/identities/email identities `1/1/0`; Kakao registry rows `0`; launch `open` with all three flags true and emergency `NULL`.

## 2026-08-25 — PHASE 10R Google-only account onboarding alignment (LOCAL / Draft)

- Classified active stale authentication contracts separately from historical evidence and the SchoolLove custom recovery boundary. Active governance and `/login` metadata now name Google as the sole user-facing provider; historical PHASE records and FROZEN documents remain unchanged.
- `/account` no longer receives or renders `auth.user.email`. It renders only the coarse label `Google 계정으로 로그인됨` and does not persist or expose Google email, provider payload, broker subject, or Auth UUID.
- Retained `/account` as the fixed post-login destination because it already exposes the four DB-authoritative stages: adult self-attestation, four required consents, one owner-only private profile, and owner-only past-school history. Instagram remains optional and private.
- Replaced the current account E2E's Supabase Email Auth, Mailpit, and six-digit OTP bootstrap with a disposable local fixture that starts from a genuine Supabase Auth session, rebinds only the disposable Auth identity to the exact `custom:schoollove-google` contract, verifies and refreshes that identity through Supabase Auth, and installs the resulting genuine access/refresh tokens through the test-only proxy. The real `/account` server boundary then validates the session through `getAuthenticatedServerContext()`. The Playwright bootstrap does not traverse `/auth/social/complete/session`; that production social-session completion boundary remains covered independently by the existing `preview-recovery-http` Vitest security contract. Anonymous Auth is enabled only inside the disposable runner; runtime validation is unchanged.
- SchoolLove custom recovery remains separate with eight-digit OTP and `USE_PRIMARY_PROVIDER`; no real Google/provider request, email, OTP, recovery delivery, Preview durable write, external configuration change, or Production mutation is authorized.
- Final local evidence: targeted Vitest `8 files / 57 tests`, full Vitest `152 passed files / 3 skipped files` and `1,224 passed tests / 4 skipped tests / 0 failed`, TypeScript, and a localhost-placeholder Next.js build (`59/59` static pages) passed. Google-only login Playwright passed `4/4`; the disposable account lifecycle passed `20/20` across desktop and mobile `360/390/412`, with `workers=1` and `retries=0`.
- Added-line scans found no credential/secret, email address, UUID literal, sensitive log, or public identity/token disclosure. Route inventory found zero changed API runtime routes, migrations, or new public routes.

## 2026-08-26 — PHASE 10U safe people-discovery readiness audit (LOCAL / Draft)

- Audited the dormant exact-person search, request/response, connection, message, block/report, Instagram, rate-limit, service-role and RLS surfaces without changing runtime, API behavior, migration, feature flags or external configuration. People discovery remains disabled.
- Resolved a disposable-only SQL transport parity defect: Windows PowerShell native stdin conversion changed non-ASCII migration regex bytes to `?`. The audit harness now byte-preserves repository SQL through `docker cp` plus container-side `psql -f`; PostgreSQL `17.6`, `standard_conforming_strings=on`, locale `en_US.UTF-8` and normalized function MD5 `cd7e6244b9484be310c85ec28d54a378` match Preview. Runtime weakening and existing-migration changes are zero.
- A fresh Supabase PostgreSQL `17.6.1.143` container created 50 disposable local Google-bound Auth/identity fixtures and completed the required 17-state matrix. Exact/empty/duplicate/self/block/suspension/hidden/deleted/cross-school/prior-terminal/connected cases, token wrong-actor/expiry/replay, target membership/profile deletion, independent two-session single-winner concurrency, RLS/FORCE/grants and IDOR all executed; the container was removed.
- Confirmed blockers include public `emergency_stopped` search/request bypass and an Instagram GET feature-gate bypass. Required pre-public fixes also include same-target-school authority, generic state contraction, requester membership and deletion/restriction rechecks, acceptance eligibility rechecks, practical greeting obfuscation coverage, a lower first-release search limit and safety responses that remain available after discovery shutdown.
- Focused audit and existing suites passed `8 files / 71 tests`; TypeScript passed after the audit-only response type assertions were made explicit. Full Vitest and build were not required because no runtime executable source changed.
- Detailed classification and the 14-point `RECOMMENDED_10V_CONTRACT` are recorded in `docs/decisions/2026-08-26-safe-people-discovery-readiness-audit.md`. PHASE 10V implementation, feature activation, Preview durable writes, live provider/email calls, external configuration and Production mutations were not performed.

## 2026-08-26 — PHASE 10V narrow people-discovery safety hardening (LOCAL / Draft)

- Started from exact `origin/preview` commit `03af176b9c47a74bdf6196febb763b9a112a59cc` in isolated branch `codex/phase-10v-narrow-people-discovery-safety-hardening`; Production `main` remained `3b0bd3898e85285944f6b6d3da64776570e43ab9`.
- Hardened PHASE 10U P0 `2/2` and P1 `8/8`: emergency discovery bypass, Instagram GET gate bypass, cross-school search, relationship-state oracle, stale requester membership, deletion-pending discovery/request, stale acceptance eligibility, practical greeting obfuscation, first-release rate, and pending safety-response feature coupling.
- Search requires the actor's current membership in the exact target school, retains graduation-year independence, returns only `match_available` plus an opaque token or one generic unavailable state, exposes no target identity, and enforces five daily attempts independently by IP and account.
- Page/API actions share the existing public-account authority where expansion is possible. DB functions independently enforce emergency, effective beta features, deletion state, target-school membership, token ownership/expiry/single-use, bilateral blocks, and prohibited prior pair state. Request creation consumes stale legitimate authority safely; acceptance atomically rechecks both parties.
- Pending-request GET and receiver decline/not-the-person/block/report remain available after discovery feature stop or emergency. Search/request/reminder/accept are blocked. Existing participant relationship reads, disconnect, block, and report remain safety-preserving; messaging and Instagram stay disabled.
- TypeScript and PostgreSQL 17 NFKC greeting checks reject the same expanded contact-exchange/obfuscation corpus, including standalone Korean `카카오`/`인스타` aliases and comma/period/exclamation/question-mark handle terminators, while allowing ordinary Korean punctuation, numbers, and a standalone `@` symbol. Instagram GET checks `instagram_permission` before conversation/private-profile lookup.
- Added only `20260826061123_people_discovery_safety_hardening.sql` (SHA-256 `B2C72FEBF19C0EAC2297FE30D1BBF39DD28DD7E19C0F2F1AD058D7E08C179F7B`). It replaces six existing functions; page/API routes, tables, columns, new functions, telemetry, providers, and historical migration edits are zero. Migration inventory is `33→34`.
- Preview migration apply, Preview DB/Auth write, feature activation, live Google/provider call, email/OTP send, external configuration mutation, Ready/merge, and Production mutation remain zero.

## 2026-08-27 — PHASE 10X people-discovery controlled-beta contract (LOCAL / Draft)

- Started from exact `origin/preview` commit `2f57d6f3d83e81f8ca61cd4fce05f241959817f4` in isolated branch `codex/phase-10x-people-discovery-controlled-beta-contract`; Production `main` remained `3b0bd3898e85285944f6b6d3da64776570e43ab9`.
- Preserved `firstControlledBetaEnabledFeatures` and the legacy `ACCOUNT_PRIVATE_BETA` exact pair. Added the order-independent `PEOPLE_DISCOVERY_BETA` exact pair and a shared classifier; every partial, mixed, duplicate, or expanded set is rejected.
- Server readiness now derives the expected enabled features from the immutable snapshot. A valid program has all eight program-level feature rows, exactly its approved pair enabled, the other six disabled, one matching school, a 20-user cap, exactly 14 days, admin approval, one-use/seven-day invites, and all three mandatory stop conditions.
- Added `20260827090000_people_discovery_controlled_beta_contract.sql`. It replaces only the six existing functions whose first-contract assumptions needed correction; table, column, new-function, route, RLS, and broad-grant deltas are zero. `admin_record_beta_readiness` and `admin_controlled_beta_stop` required no replacement.
- Effective feature access remains member-, program-, time-, school-, snapshot-, program-flag-, optional-user-deny-, global-stop-, and suspension-bound. A global `people_search` stop also denies dependent `connection_request`; PHASE 10V search/request/reminder/accept stay fail-closed while receiver safety closure is unchanged.
- Disposable PostgreSQL verifies the fresh 35-migration chain and the 34→35 upgrade, the unchanged PHASE 10J account/private lifecycle, the people-search/request lifecycle through invite and admin approval, mixed/snapshot/start/school negative cases, PHASE 10V exact search→request→accept, stop/emergency behavior, and service-only admin grants. All disposable writes roll back and the container is removed.
- This phase does not apply the migration or create any beta draft, program, snapshot, flag, invite, member, connection, or Auth user in Preview. People discovery, messaging, and Instagram remain disabled; live provider/email, external configuration, and Production mutations are zero.

## 2026-08-31 — Production social-login bootstrap gate (LOCAL)

- Added `production-bootstrap` to the existing server-owned broker exposure switch. It resolves only to the immutable Production issuer/profile and is rejected outside `VERCEL_ENV=production`.
- Only OIDC discovery and JWKS remain available for Supabase provider bootstrap, while `/login` hides the Google CTA and Google start/callback, custom recovery, social completion, authorization, and token endpoints all stay fail-closed. Preview remains fully available under its existing `preview` exposure; final Production login requires the distinct `production` value.
- Recorded the zero-broken-window order in `docs/decisions/2026-08-31-production-social-login-bootstrap-gate.md`. No new environment variable, migration, database/Auth mutation, provider call, Google/Vercel/Supabase setting, commit, push, deployment, or Production login was performed.
- Verification passed targeted `5 files / 66 tests`, full Vitest `171 passed files / 3 skipped files` and `1,412 passed tests / 4 skipped tests`, TypeScript, ESLint with `0 errors` and the existing `86 warnings`, and the 58-page Next.js Production build.

## 2026-09-10 — Game visual polish V1 baseline (ASSET_INPUT_REQUIRED)

- Created local `codex/game-visual-polish-v1` from canonical Preview PR112 commit `85686b49d222109c9d8f947679733b32cde504fe`. The user's follow-up correction requires the original mockup's cute boy/girl duo, not three adult alumni; recorded in the decision document.
- Added an isolated local production-build lab, fixed CPU/network browser measurement and 390/1440 guest Home baseline captures. Unchanged baseline build passed; five fresh-cache LCP runs were 2592/2972/2628/2640/2828 ms (median 2640), three warm runs 448/488/460 ms (median 460). Historical 3.98 s is not directly comparable; no After or improvement claim.
- Generated character/stage candidates did not pass real-alpha verification. Targeted background extraction and another stage generation encountered a built-in image editor reference-file sandbox-helper decoding error. No alternate CLI/API/tool route was used. Runtime source/CSS/public assets remain unchanged; visual completion and release gates remain unpassed.
- Raw local traces, screenshots, prompt/provenance notes, exact failed targets and requested input dimensions are retained in ignored `.local/game-visual-polish/STATUS.md` and adjacent evidence. No commit/push/PR/Preview release or Production change was performed for this follow-up. Resume after image-tool reference access recovery or valid real-alpha asset input.

## 2026-09-10 — Game visual polish V1 continuation (LOCAL_VERIFIED / PARTIAL)

- Following the user's approval, built-in image editing produced a valid-alpha original-style boy/girl duo, reunion scene and lively garden/gazebo scene. Four campus forms now exist; the fifth arch repeatedly failed alpha and is excluded. No realistic adult-three-person asset is used. The five-form and canonical-release gates remain unpassed.
- Added local pause/resume, reduced-motion preference changes, offscreen/document-visibility handling, mode-specific sizes and a typed Home AVIF source with WebP fallback. No growth/projection/access/referral calculation changed; only the two approved microcopy adjustments accompany presentation changes.
- Final targeted35 and full1704 tests passed (4 existing skipped), typecheck/build passed, lint0 errors/84 existing warnings. Actual React responsive61, mocked functional17, long-name/multiple-card/image-failure checks and motion keyboard/offscreen/reduced-motion passed. Document-hidden is synthetic; physical mobile devices and physical200% browser zoom remain NOT_TESTED.
- Same mobile lab fresh-cache LCP median: Before2640ms; first WebP candidate2676ms (failed); final AVIF1668ms (2.5s budget passed). Warm medians460/504/628ms respectively: final warm regression remains explicit. CLS0.00808317 fresh,0 warm. All runs/traces/screenshots retained; no claim about the historical3.98s cause or field CWV.
- Original schoolWebPs, all API/lib/DB/migration/XP/referral/auth rules, packages/lockfiles/env/protection and A/B/C/relationships remain untouched. No explicit live user-data write, real provider login, school save, person search, referral issue or external sharing.
- `docs/GAME_VISUAL_POLISH_V1_RESULT.md` and ignored evidence index contain the current record. Feature Draft review is authorized; Ready/canonical merge is withheld for missing fifth form, warm regression and remaining verification gates. Production is not approved or changed.

## 2026-09-10 — Game visual polish Finish V1 (LOCAL_VERIFIED / PARTIAL)

- Continued the same branch from307ba36. Only runtime change imports the identical Home AVIF into Next's content-hashed immutable asset pipeline. Original cute duo, four forms, CSS/motion, all app/API/lib/data/auth/XP/referral boundaries and packages remain unchanged.
- One precise final arch edit and one background correction both returned RGB checkerboards without alpha; rejected inputs/composites retained outside runtime. Fifth form remains ASSET_REQUIRED; no image-tool workaround.
- One fixed-protocol current Before/After batch: fresh median1612→1468ms, warm576→552ms. Warm AVIF304 revalidation179–190ms became zero-transfer cached200/0ms load, but residual layout/render work persists. Historical PR112 warm460ms remains better: WARM_CACHE_REVIEW_REQUIRED. Individual CLS increases remain recorded, all≤0.1. All historical/raw runs retained.
- Targeted41, full1705PASS/4existingSKIP, typecheck, lint0errors/84existingwarnings and67-page production build passed. Actual React responsive61, synthetic functional17, final image-failure/long-name/two-card checks and motion checks passed. Initial image-failure harness incorrectly aborted Vite JS imports and was corrected without weakening image assertions; failures retained. Encoder EPERM resolved through formal same-command permission, not another encoder. Physical devices/real background tab remain untested.
- GitHub existing connector PR-write restoration unconfirmed; metadata lists are empty and browser settings requires login. No PR retry/alternate credential route. User separately authorized one official final-feature review link, with no URL/cookie persistence or project-wide protection changes. Final push/deployment/live status belongs to the ignored final report; this source record does not claim it prematurely.
- See `docs/GAME_VISUAL_POLISH_FINISH_V1_RESULT.md` and `.local/game-visual-polish/finish`. Ready/canonical merge remains withheld, Production untouched.

## 2026-09-10 — Independent final-stage campus layer after c538e02 (LOCAL_VERIFIED)

- Under the user's new explicit implementation approval, added original `SchoolMemoryGate` SVG/DOM geometry over unchanged lively-school art. No failed-raster processing, image-generation retries or background-removal/alternate API. Four raster sources plus the final structural layer now yield five visually distinct local scenes; original cute duo and stage/growth semantics remain unchanged.
- Equal500px stationary/grayscale-effects-off comparison and white/pastel/navy scene composites inspected. Browser SVG rasterization has real alpha, transparent opening/corners and95.81874% fully transparent pixels. Final Hub/owner360/390/412/768/1280/1440 show no overflow or gate/duo overlap. One shared animated wrapper preserves pause/resume/reduced/offscreen without new observers. SSR IDs are unique across multiple instances.
- Targeted45 and full1710PASS/4existingSKIP, typecheck, lint0errors/84existingwarnings,67-page production build passed. Existing responsive61 and synthetic functional17 passed. Actual compiled Home/level10Hub SSR+hydration+motion passed with0 page errors using fixed local transport. Vite capture's initial reload race was corrected with network-idle synchronization, not a runtime workaround.
- No LCP remeasurement/iterative optimization. Historical c538e02 warm552ms remains+92ms against original460ms and awaits user acceptance. Identical immutable Home AVIF preserved. New inline SVG4489B/gzip1579B; compiled Home decoded JS657595→662757B (+5162), rounded First Load JS180→182kB. Cost disclosed as design/performance review, not no-regression PASS.
- Runtime scope: SchoolWorld, new SVG component and six scoped CSS lines. No DB/API/lib/auth/XP/referral/env/protection/package or real-user change. Same-branch commit/push authorized; PR-write restoration and browser network restoration remain separate unconfirmed blockers. No PR retry/new review link/Ready/Preview merge/Production action. See `docs/GAME_VISUAL_REMAINING_GATES_RESULT.md` and ignored `.local/game-visual-polish/remaining` evidence/manifest.

## 2026-09-10 — Accepted V1, normal GitHub web Draft and caption spacing

- Latest user acceptance resolves the preceding historical design/performance-decision pending states. Preserve c538e02 warm552ms versus PR112460ms (+92ms) and942a305 decoded-JS+5162B as accepted costs, not no-regression PASS or new timing data.
- User-authenticated owner web UI created Preview-base Draft PR113 once after zero duplicate matches. Existing connector write scope is still unconfirmed; no API creation retry, token/protection change or new review link. Exact942a305 feature rendered through the normal in-app browser; Home360/390/1440, public school390/1440 and closed login390 evidence retained separately from synthetic Lv.10/private evidence.
- Actual PC School Hub motion button overlapped the caption16.29px. Added only a24px desktop caption top margin, preserving art, duo, scale, stage and motion logic. Local compiled synthetic Lv.10 verifies non-overlap at390/1024/1280/1440;1440 gap7.71px (before -16.29px), gate retained, no horizontal overflow. One1280 image sample was still incomplete immediately after responsive resize; no blanket all-image-ready claim for that sample.
- Fresh validation of this CSS candidate: targeted2files/30PASS; typecheckPASS; full206files/1710PASS,3files/4tests existingSKIP; lint0errors/84existingwarnings; actual Next15.3.8 production buildPASS,67static pages, with fixed local transport and remote-fetch guard. Existing five-stage/alpha/private/reduced-motion evidence is reused because those assets/components/logic are unchanged; no new performance batch.
- Only app/game.css and three existing documentation files change from942a305. Final corrected feature-head live checks, PR Ready/merge and canonical postflight are still required and will be recorded on PR113 and the ignored final report. Production/main, DB/auth/XP/referral and actual-user writes remain untouched.
# 2026-09-15 — Approved fantasy V3 application
...
## 2026-09-16 — Privacy release production verification

- `PRODUCTION_VERIFIED`: PR #119 merged as `5daeaab5b110534a56e979253fbd0b9184c2c6d6`; Vercel `dpl_Gihig63yqXmFMhkU1gEYjNawihAc` READY with production aliases. Live privacy/login pages HTTP200, pre-login notices and officer telephone verified at mobile width390; anonymous admin API401; no production browser errors.
- Supabase maintenance recovered early. Applied versions `20260915211712` and `20260915211726` confirmed. Cron succeeded at KST06:30/06:35/06:40; expired login attempts56→0; profiles3, memberships2, schools10006 and launch open preserved. Cleanup executable only by service role/DB owner; aggregate RLS enforced with no direct external SELECT.
- Local migration filenames aligned with actual remote versions without changing either SQL hash. Security advisor WARN counts unchanged; expected private aggregate RLS/no-policy INFO added. Real production signup/email/deletion tests not run. Temporary local servers stopped.
- Automatic review initially rejected supplementary Git history changes. The user then explicitly approved commit/push/merge of the final verification report and migration filename alignment. Under that additional approval, synchronized the existing release branch with the identical production tree. SQL hashes remain unchanged; no new runtime/DB mutation. Diff whitespace and hash checks cover this documentation/filename-only follow-up; prior full application checks were not rerun. Unrelated SNS/Home changes remain uncommitted and preserved.

## 2026-09-16 — Home hero clarity and unobstructed characters

- Implemented the user's screenshot feedback: lighten the purple artwork overlay, request quality90 at cover-plane-aware image sizes, and reserve clear mobile space for both characters. Shortened the hint to “안부를 보내고, 서로 수락하면 연결돼요.” and moved it above the action buttons.
- `LOCAL_VERIFIED`: real-component browser checks at320/360/390/412/768/1280 passed for one-line hint, no horizontal overflow and no character overlap. Mobile/desktop screenshots inspected; pause/resume preserved and browser errors0. Targeted26 tests, typecheck, full208files/1719tests (existing3files/4skips), final67page build and diff checks passed.
- Default fork execution was slow; full tests passed using thread workers. A temporary Next thread-worker build hit DataCloneError; final build passed with standard page workers and local in-process compilation. Original next.config.ts restored exactly. No test exclusions or application config change retained.
- Preserved source artwork/logo, headline, routes, motion/privacy/feature contracts, existing SNS/Home work and completed privacy release. Local implementation made no remote mutation, package change or environment-file edit. The user subsequently explicitly approved commit/push/merge/production deployment with “응 응”; production optimizer verification follows release. Physical phone checks remain unavailable. Evidence: docs/verification/2026-09-16-home-hero-clarity.md.
