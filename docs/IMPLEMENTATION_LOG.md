...
- TypeScript and PostgreSQL 17 NFKC greeting checks reject the same contact-exchange/obfuscation corpus while allowing `나 완이야. 오랜만이야.`. Instagram GET checks `instagram_permission` before conversation/private-profile lookup.
- Added only `20260826061123_people_discovery_safety_hardening.sql` (SHA-256 `272F40A0C7A14AB127E7FDCCB6948A38B898E48842C2F1E463C97E4E7E60AE55`). It replaces six existing functions; page/API routes, tables, columns, new functions, telemetry, providers, and historical migration edits are zero. Migration inventory is `33→34`.
