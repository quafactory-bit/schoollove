...
- Preserved the Google progression: first login under `closed` became recovery-verified and Auth-principal-bound; one bound-provisional reauthentication completed without recovery; an approved Preview-only Vercel Authentication window allowed one final Google flow after discovery/JWKS had initially been blocked; `/account` was reached and protection was restored immediately.
