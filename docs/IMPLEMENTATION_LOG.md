...
- Implemented the exact opaque subject contract, independent one-time state/nonce digest bindings, S256-only PKCE, immutable-provider/terminal login attempts, fake Kakao/Naver/Google adapters, ephemeral Ed25519 fake OIDC metadata/JWKS/code/ID-token harness, and whitelist-only safe logging.
...
- Frozen the no-email-linking policy, Auth-email-NULL direction, immutable primary provider, separate recovery-email OTP boundary, and exact local-part-preserving/domain-only-IDNA canonicalization in `docs/decisions/2026-08-10-social-auth-broker-contract.md`.
