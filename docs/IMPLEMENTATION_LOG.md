...
- Implemented the exact opaque subject contract, independent one-time state/nonce digest bindings, S256-only PKCE, immutable-provider/terminal login attempts, fake Kakao/Naver/Google adapters, ephemeral in-memory RSA test-key RS256 OIDC metadata/JWKS/code/ID-token harness, and whitelist-only safe logging.
