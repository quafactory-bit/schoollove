...
- Replaced the current account E2E's Supabase Email Auth, Mailpit, and six-digit OTP bootstrap with a disposable local fixture that starts from a genuine Supabase anonymous session, rebinds only the disposable Auth row to the exact `custom:schoollove-google` identity shape, and passes the genuine access/refresh tokens through the production social-session completion validator. Anonymous Auth is enabled only inside the disposable runner; runtime validation is unchanged.
