...
The token endpoint accepts only exact `application/x-www-form-urlencoded`, optionally with one case-insensitive `charset=UTF-8` parameter, and `authorization_code` requests. Unknown, duplicate, or malformed media-type parameters fail closed. It supports exactly one of `client_secret_basic` or `client_secret_post`; missing or dual methods fail before authorization-code lookup. Basic credentials are base64-decoded, split once on the colon, then each component is URL-query-unescaped exactly once to match the pinned `x/oauth2` shape. Client secrets are verified through a domain-separated fixed-length digest using constant-time comparison; raw secrets are not logged, persisted, returned, or committed.
