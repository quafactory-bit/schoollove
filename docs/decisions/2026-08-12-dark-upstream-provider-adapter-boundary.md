...
The adapter is a dark, process-local protocol harness, not Production-ready callback persistence. Its pending state, nonce binding, and PKCE verifier disappear when the adapter instance/process disappears. Before any browser redirect or Production provider wiring, a separately approved phase must move C-leg state/nonce digests and the PKCE verifier to an attempt-bound durable/resumable boundary. An adapter instance must not be used as a Production route singleton or session state.
...
Provider endpoints are exact internal pins, not caller configuration: Kakao uses only the documented `kauth.kakao.com` authorization/token/JWKS URLs, Google uses only the documented `accounts.google.com`/`oauth2.googleapis.com`/Google JWKS URLs, and Naver uses only the documented NID authorization/token/profile URLs. HTTPS alone is insufficient. Transport response URLs must equal the pinned URL; endpoint substitution fails closed.
...
Authorization codes are opaque provider values. The adapter preserves their once-decoded query value exactly, allows bounded printable values up to 2048 UTF-8 bytes, and rejects control characters. It does not impose a URL-safe alphabet, lowercase, normalize, or decode again.
