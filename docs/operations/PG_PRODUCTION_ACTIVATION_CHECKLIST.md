...
- [ ] `PAYMENT_PROVIDER_MODE=portone_sandbox`와 `NEXT_PUBLIC_PAYMENT_PROVIDER_MODE=portone_sandbox`를 별도 승인 후 함께 설정
...
- [ ] provider store ID·TEST channel·customData order ID 불일치 거부 재검증
- [ ] provider 호출 전 환불 한도 예약과 중복/초과 환불 차단 재검증
