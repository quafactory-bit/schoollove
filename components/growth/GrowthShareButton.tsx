...
export default function GrowthShareButton({ schoolId, schoolName, slug }: { schoolId: string; schoolName: string; slug: string }) {
  ...
  const invitation = `${schoolName}, 우리 학교 친구들, 다시 찾아볼래?\n연결 후 서로 허용한 인스타그램주소를 확인할 수 있어.`
  useEffect(() => {
  ...
  return <div>
    ...
    <dialog ref={dialog} aria-labelledby={titleId} className="growth-journey sl-game sl-share-dialog m-auto max-h-[85dvh] w-[calc(100%-2rem)] max-w-md overflow-y-auto rounded-2xl border border-schoollove-border bg-white p-5 text-schoollove-text shadow-xl backdrop:bg-black/40 sm:p-6">
      ...
      <div className="sl-share-preview mt-4 break-words p-4"><p className="text-sm font-bold">SchoolLove · 기억 속 친구와 다시 연결돼요</p><SceneImage scene="share" className="sl-scene--share" sizes="440px" /><p className="sl-share-headline">친구와 함께<br />더 높이 올라가요!</p><h3 className="mt-3 text-lg font-bold">{schoolName}</h3><p className="mt-2 text-sm leading-6">{invitation}</p><p className="mt-3 text-sm text-schoollove-secondary">공유에는 학교 이름과 공개 학교 링크만 들어가요. 내 이름·졸업연도·반·개인 참여 정보는 넣지 않아요.</p></div>
      <p className="mt-4 text-sm leading-6">미리보기만 열면 링크를 만들지 않아요. 친구 링크는 준비 후 최대 7일간 유효해요. 같은 학교 신규 참여 조건을 충족할 때만 XP가 반영되며, 복사·공유만으로 XP가 생기지 않아요. 친구 초대와 사람 찾기 이용 승인은 별개예요.</p>
      ...
      <p className="text-xs leading-5 text-schoollove-secondary">일반 링크에는 추천 정보가 없으며 추천 XP 보상이 연결되지 않아요.</p>
