...
export default function GrowthShareButton({ schoolId, schoolName, slug }: { schoolId: string; schoolName: string; slug: string }) {
  ...
  return <div>
    ...
    <dialog ref={dialog} aria-labelledby={titleId} className="growth-journey m-auto max-h-[85dvh] w-[calc(100%-2rem)] max-w-md overflow-y-auto rounded-2xl border border-schoollove-border bg-white p-5 text-schoollove-text shadow-xl backdrop:bg-black/40 sm:p-6">
      ...
      <div className="mt-4 break-words rounded-xl bg-[var(--schoollove-game-surface)] p-4"><p className="text-xs font-bold text-[var(--schoollove-game-accent)]">스쿨러브아이 · 우리 학교 함께 키우기</p><h3 className="mt-2 text-lg font-bold">{schoolName}</h3><p className="mt-2 text-sm leading-6">{invitation}</p><p className="mt-3 text-xs text-schoollove-secondary">공유에는 학교 이름과 공개 학교 링크만 들어가요. 내 이름·졸업연도·반·개인 성장 수치는 넣지 않아요.</p></div>
