...
export default async function MyGrowthSchools() {
  try {
    ...
    return <section className="border-t border-schoollove-border py-10" aria-labelledby="my-growth"><h2 id="my-growth" className="text-2xl font-bold">내가 함께 키우는 학교</h2><div className="mt-6 grid gap-5 md:grid-cols-2">{schools.map(school => <article key={school.schoolId} className="min-w-0 border border-schoollove-border p-6"><Link href={`/school/${encodeURIComponent(school.slug)}`} className="schoollove-focus break-words text-xl font-bold">{school.schoolName}</Link><GrowthMeter growth={school} projection="owner" /><div className="mt-5"><GrowthShareButton schoolId={school.schoolId} schoolName={school.schoolName} slug={school.slug} /></div></article>)}</div></section>
  } catch { return null }
}
