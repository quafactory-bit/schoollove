    ...
    if (schoolSlug) {
      supabase.from('schools').select('*').eq('slug', schoolSlug).single().then(({ data: school }) => {
        if (school) {
          ...
          if (year) {
            setGraduationYear(parseInt(year))
            if (gradeParam) setGrade(parseInt(gradeParam))
            if (classParam) setClassNumber(parseInt(classParam))
            if (year && gradeParam && classParam) {
              setStep(3)
            } else if (year) {
              setStep(2)
            }
    ...
    <div className="space-y-6">
      ...
      {step === 1 && (
        <div className="card p-5 space-y-4">
          ...
          <button onClick={goStep2} disabled={!selectedSchool} className="btn-primary w-full text-sm">
            다음
          </button>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="card p-5 space-y-4">
          ...
          <p className="text-sm text-brand-blue font-medium">{selectedSchool?.school_name}</p>

          <div className="space-y-1.5">
            <label className="text-xs font-medium text-gray-600">졸업(예정) 년도 <span className="text-red-500">*</span></label>
            <div className="relative">
              <select value={graduationYear} onChange={(e) => setGraduationYear(Number(e.target.value))} className="input-base appearance-none pr-8 cursor-pointer">
          ...
          {!isUniv && grades.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-gray-600">학년 <span className="text-red-500">*</span></label>
                <div className="relative">
                  <select value={grade} onChange={(e) => setGrade(Number(e.target.value))} className="input-base appearance-none pr-8 cursor-pointer">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-gray-600">반 <span className="text-red-500">*</span></label>
                <input type="number" min={1} max={30} placeholder="반 번호" value={classNumber} onChange={(e) => setClassNumber(Number(e.target.value))} className="input-base" />
              </div>
            </div>
          )}
          ...
          {isUniv && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <input type="text" placeholder="예: 컴퓨터공학과" value={department} onChange={(e) => setDepartment(e.target.value)} className="input-base" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-gray-600">학번 (선택)</label>
                <input type="number" placeholder="예: 19" value={studentYear} onChange={(e) => setStudentYear(Number(e.target.value))} className="input-base" />
              </div>
            </div>
          )}

          <button onClick={goStep3} disabled={!graduationYear || (!isUniv && (!grade || !classNumber))} className="btn-primary w-full text-sm">
            다음
          </button>
        </div>
      )}
          ...
          <div className="space-y-3">
            ...
            {rows.map((row, idx) => (
              <div key={row.id} className="space-y-1">
                ...
                {row.nicknameError && <p className="text-xs text-red-500 px-1">{row.nicknameError}</p>}
              </div>
            ))}
          </div>

          <button onClick={addRow} className="w-full flex items-center justify-center gap-1.5 py-2.5 border border-dashed border-gray-300 rounded-xl text-sm text-gray-500 hover:border-brand-blue hover:text-brand-blue transition-colors">
            <Plus size={16} />
            친구 추가
          </button>
