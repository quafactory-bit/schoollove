...
BEGIN
  ...
  IF has_function_privilege('service_role','public.create_broker_authorization_code(uuid,uuid,bytea,text,text,text,bigint,bytea,bytea,bytea,integer)','EXECUTE')
    OR NOT has_function_privilege('service_role','public.issue_transaction_bound_broker_authorization_code(uuid,uuid,bytea,bigint,text,bytea,bytea,bytea,integer)','EXECUTE') THEN RAISE EXCEPTION 'PHASE10O_P_SERVICE_RPC'; END IF;
