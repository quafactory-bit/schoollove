$ErrorActionPreference='Stop'; $containerName='schoollove-phase10o-p-db'; $image='public.ecr.aws/supabase/postgres:17.6.1.143'; $created=$false; $testPassword=('phase10op_'+[guid]::NewGuid().ToString('N'))
function Invoke-SqlFile([string]$file){Get-Content -LiteralPath $file -Raw -Encoding UTF8|docker exec -i $containerName psql -U postgres -d phase10op -v ON_ERROR_STOP=1 -q;if($LASTEXITCODE-ne 0){throw "SQL failed: $file"}}
function Invoke-Sql([string]$sql){$sql|docker exec -i $containerName psql -U postgres -d phase10op -v ON_ERROR_STOP=1 -q;if($LASTEXITCODE-ne 0){throw 'SQL bootstrap failed.'}}
try {
  ...
  docker exec $containerName createdb -U postgres phase10op
  Invoke-Sql @"
...
"@
  ...
  Invoke-SqlFile (Resolve-Path 'scripts/phase10o-j/lifecycle-smoke.sql').Path
  ...
  # Earlier isolated J/N/O regressions intentionally retain social fixture ledgers.
  # The P acceptance starts from the same empty relevant-ledger baseline verified for Production.
