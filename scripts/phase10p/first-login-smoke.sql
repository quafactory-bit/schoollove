...
DECLARE
  ...
  transaction_id uuid:=gen_random_uuid(); leg_id uuid:=gen_random_uuid(); code_id uuid:=gen_random_uuid(); auth_id uuid:=gen_random_uuid(); other_auth_id uuid:=gen_random_uuid();
...
BEGIN
  ...
  INSERT INTO auth.users(id,email) VALUES(auth_id,NULL),(other_auth_id,NULL);
  INSERT INTO auth.identities(id,user_id,provider_id,provider,identity_data) VALUES(gen_random_uuid(),auth_id,subject_value,'schoollove-google',jsonb_build_object('sub',subject_value)),(gen_random_uuid(),other_auth_id,'wrong-subject','schoollove-google',jsonb_build_object('sub','wrong-subject'));
