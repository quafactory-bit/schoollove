...
$token=(docker exec $ContainerName psql -U postgres -d $DatabaseName -tAc 'SELECT match_token FROM phase10v_audit.concurrency_fixture LIMIT 1').Trim()
if($LASTEXITCODE-ne 0-or-not$token){throw 'Concurrency token fixture is missing.'}
$actor='00000000-0000-4000-8000-000000000073'
...
$created=(docker exec $ContainerName psql -U postgres -d $DatabaseName -tAc "SELECT count(*) FROM public.connection_requests WHERE sender_user_id='$actor'::uuid AND receiver_user_id='00000000-0000-4000-8000-000000000074'::uuid").Trim()
...
if($LASTEXITCODE-ne 0-or$created-ne'1'-or$used-ne'1'){throw "Concurrency failed: requests=$created used=$used outputs=$($outputs-join',')"}
Write-Output 'PHASE10V_MATCH_TOKEN_CONCURRENCY_OK requests=1 used_tokens=1'
