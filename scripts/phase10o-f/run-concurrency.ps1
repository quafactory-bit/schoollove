...
$challengeA=docker exec $ContainerName psql -U postgres -d phase10of -tAc "SELECT v.id FROM private.recovery_email_verifications v JOIN private.private_accounts a ON a.id=v.account_id WHERE a.primary_provider='kakao'"
$challengeB=docker exec $ContainerName psql -U postgres -d phase10of -tAc "SELECT v.id FROM private.recovery_email_verifications v JOIN private.private_accounts a ON a.id=v.account_id WHERE a.primary_provider='naver'"
...
Write-Output 'PHASE10O_F_CONCURRENCY_OK broker=1 recovery_hmac=1 consume=1'
