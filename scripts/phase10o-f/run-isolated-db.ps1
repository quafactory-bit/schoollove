...
"@
  ...
  if($env:PHASE10O_I_ACCEPTANCE -eq '1'){
    Invoke-Sql 'TRUNCATE private.recovery_email_verifications,private.social_identity_registry,private.oauth_login_attempts,private.auth_principal_cleanup_jobs,private.private_accounts CASCADE; DELETE FROM auth.users;'
    Invoke-SqlFile (Resolve-Path 'supabase/migrations/20260811110000_recovery_delivery_state_boundary.sql').Path
    foreach($smoke in @('scripts/phase10o-i/lifecycle-smoke.sql','scripts/phase10o-i/permissions-smoke.sql')){Invoke-SqlFile (Resolve-Path $smoke).Path}
    Invoke-SqlFile (Resolve-Path 'scripts/phase10o-i/concurrency-setup.sql').Path
