...
$env:NEXT_PUBLIC_SUPABASE_ANON_KEY='phase10i-local-anon'
$env:SUPABASE_SERVICE_ROLE_KEY='phase10i-local-service'
$env:PUBLIC_PROFILE_REGISTRATION_ENABLED='false'
$env:ADMIN_PASSWORD='phase10i-local-admin-password'
...
try {
  ...
  if($server.HasExited){throw 'Next.js PHASE 10I E2E server exited during startup.'}
  & node.exe node_modules/@playwright/test/cli.js test e2e/phase10i-operations.spec.ts --workers=1
