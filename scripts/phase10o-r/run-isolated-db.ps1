$ErrorActionPreference='Stop'; $containerName='schoollove-phase10o-r-db'; $image='public.ecr.aws/supabase/postgres:17.6.1.143'; $created=$false; $testPassword=('phase10or_'+[guid]::NewGuid().ToString('N'))
function Invoke-SqlFile([string]$file){Get-Content -LiteralPath $file -Raw -Encoding UTF8|docker exec -i $containerName psql -U postgres -d phase10or -v ON_ERROR_STOP=1 -q;if($LASTEXITCODE-ne 0){throw "SQL failed: $file"}}
function Invoke-Sql([string]$sql){$sql|docker exec -i $containerName psql -U postgres -d phase10or -v ON_ERROR_STOP=1 -q;if($LASTEXITCODE-ne 0){throw 'SQL bootstrap failed.'}}
try {
  ...
  docker exec $containerName createdb -U postgres phase10or
  Invoke-Sql @"
...
"@
  ...
  $mapping=(docker port $containerName 5432/tcp).Trim();if($mapping -notmatch ':(\d+)$'){throw 'Host TCP port discovery failed.'};$env:PGHOST='127.0.0.1';$env:PGPORT=$Matches[1];$env:PGDATABASE='phase10or';$env:PGUSER='postgres';$env:PGPASSWORD=$testPassword
  node scripts/phase10o-r/race-runner.mjs;if($LASTEXITCODE-ne 0){throw 'PHASE 10O-R direct-TCP concurrency acceptance failed.'}
  Remove-Item Env:PGHOST,Env:PGPORT,Env:PGDATABASE,Env:PGUSER,Env:PGPASSWORD -ErrorAction SilentlyContinue
  $tableCount=(docker exec $containerName psql -U postgres -d phase10or -tAc "SELECT count(*) FROM pg_catalog.pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='private' AND c.relkind='r'").Trim();if($tableCount-ne'9'){throw "Private table boundary mismatch: $tableCount"}
  Write-Output 'PHASE10O_R_ISOLATED_DB_OK private_tables=9 container_removed=true'
