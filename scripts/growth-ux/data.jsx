...
const visualLevel = Number(new URLSearchParams(location.search).get('level')) || 1
const publicGrowth = {schoolId:school.id,schoolName:school.school_name,slug:school.slug,level:visualLevel,progress:visualLevel>1?65:0,rank:null,lastLevelUp:null}
...
export const getSchoolGrowth=async(id)=>({status:'ok',schools:id?[publicGrowth]:new URLSearchParams(location.search).get('ranking')==='populated'?[{...publicGrowth,rank:1}]:[]})
export const getOwnSchoolGrowth=async()=>({...publicGrowth,ownContributionXp:0})
