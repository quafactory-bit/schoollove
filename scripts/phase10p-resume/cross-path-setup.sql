...
DO $$
DECLARE
  digest bytea:=decode(repeat('f1',32),'hex');
  subject text:='slb:v1:k01:google:'||translate(rtrim(encode(digest,'base64'),'='),'+/','-_');
  source_attempt uuid;
  source_tx uuid:='77000000-0000-4000-8000-000000000001';
  source_leg uuid:='77000000-0000-4000-8000-000000000002';
  verification uuid:='77000000-0000-4000-8000-000000000003';
  account uuid:='77000000-0000-4000-8000-000000000004';
  source_code uuid:='77000000-0000-4000-8000-000000000005';
  outcome text;
BEGIN
  -- This orphan naturally expires while the preceding cross-path race runs.
  source_attempt:=public.create_social_login_attempt('att_10p_wait_source_01','google',clock_timestamp()+interval '12 seconds');
  PERFORM public.create_downstream_authorization_transaction(source_tx,source_attempt,decode(repeat('f2',32),'hex'),'slb-supabase-google','https://hukokfyphyrpfouazxhq.supabase.co/auth/v1/callback','code','openid',repeat('U',43),'S256',NULL,'wait-source',clock_timestamp()+interval '11 seconds');
  PERFORM public.claim_downstream_authorization_transaction_by_handle(decode(repeat('f2',32),'hex'));
  PERFORM public.create_upstream_login_leg(source_attempt,source_leg,'google',decode(repeat('f3',32),'hex'),decode(repeat('f4',32),'hex'),decode(repeat('f5',32),'hex'),repeat('V',43),decode(repeat('f6',17),'hex'),decode(repeat('f7',12),'hex'),1);
  PERFORM public.bind_downstream_authorization_transaction_upstream_leg(source_tx,source_leg);
  PERFORM public.claim_upstream_login_callback_by_state('google',decode(repeat('f3',32),'hex'),decode(repeat('f4',32),'hex'));
  IF public.record_verified_social_identity_from_upstream_leg(source_attempt,source_leg,'google',subject,digest,1)<>'RECOVERY_REQUIRED' THEN
    RAISE EXCEPTION 'PHASE10P_WAIT_SOURCE_IDENTITY';
  END IF;
  SELECT x.outcome INTO outcome FROM public.create_and_reserve_login_attempt_recovery_delivery(source_attempt,verification,account,decode(repeat('f8',32),'hex'),1,decode(repeat('f9',17),'hex'),decode(repeat('fa',12),'hex'),1,decode(repeat('fb',32),'hex'),1) x;
  IF outcome<>'RECOVERY_DELIVERY_RESERVED' THEN RAISE EXCEPTION 'PHASE10P_WAIT_SOURCE_RECOVERY'; END IF;
  PERFORM public.mark_login_attempt_recovery_delivery_sent((SELECT id FROM private.recovery_delivery_attempts WHERE verification_id=verification));
  PERFORM public.consume_recovery_and_decide_social_account(source_attempt,verification,decode(repeat('fb',32),'hex'));
  SELECT x.outcome INTO outcome FROM public.issue_transaction_bound_broker_authorization_code(source_tx,source_code,decode(repeat('fc',32),'hex'),floor(extract(epoch FROM clock_timestamp()))::bigint-1,NULL,NULL,NULL,NULL,NULL) x;
  IF outcome<>'AUTHORIZATION_CODE_CREATED' THEN RAISE EXCEPTION 'PHASE10P_WAIT_SOURCE_CODE'; END IF;
END $$;

SELECT 'PHASE10P_PROVISIONAL_RESUME_CROSS_PATH_SETUP_OK' AS status;
SELECT 'PHASE10P_PROVISIONAL_RESUME_TARGET_EXPIRY_SOURCE_SETUP_OK' AS status;
