...
$containerName = 'schoollove-phase10h-db-script'
...
try {
  ...
  docker run -d --name $containerName -e POSTGRES_PASSWORD=local_phase10h_only $image | Out-Null
  ...
  for ($attempt = 0; $attempt -lt 120; $attempt++) {
    ...
    $readinessErrorPreference = $ErrorActionPreference
    $ErrorActionPreference = 'SilentlyContinue'
    docker exec $containerName psql -U postgres -d postgres -tAc 'SELECT 1' 2>$null | Out-Null
    $readinessExitCode = $LASTEXITCODE
    $ErrorActionPreference = $readinessErrorPreference
    if ($readinessExitCode -eq 0 -and ($health -eq 'healthy' -or $health -eq 'none')) {
  ...
  foreach ($smoke in @('scripts/phase10h/lifecycle-smoke.sql','scripts/phase10h/permission-smoke.sql')) {
  ...
  Write-Output 'PHASE10H_ISOLATED_DB_OK'
}
finally {
  if ($containerCreated) {
