...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM private.downstream_authorization_transactions WHERE login_attempt_id=attempt_id AND status='expired' AND broker_handle_digest IS NULL AND continuation_handle_digest IS NULL AND downstream_nonce IS NULL AND downstream_state IS NULL AND terminal_at IS NOT NULL) THEN RAISE EXCEPTION 'PHASE10P_EXPIRY_ONE_TIME_TX'; END IF;
