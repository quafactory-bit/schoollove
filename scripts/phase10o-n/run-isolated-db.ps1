$ErrorActionPreference='Stop'; $containerName='schoollove-phase10o-n-db'; $image='public.ecr.aws/supabase/postgres:17.6.1.143'; $created=$false
function Invoke-SqlFile([string]$file){Get-Content -LiteralPath $file -Raw -Encoding UTF8|docker exec -i $containerName psql -U postgres -d phase10on -v ON_ERROR_STOP=1 -q;if($LASTEXITCODE-ne 0){throw "SQL failed: $file"}}
function Invoke-Sql([string]$sql){$sql|docker exec -i $containerName psql -U postgres -d phase10on -v ON_ERROR_STOP=1 -q;if($LASTEXITCODE-ne 0){throw 'SQL bootstrap failed.'}}
try {
  ...
  docker run -d --name $containerName -e POSTGRES_PASSWORD=local_phase10on_only $image|Out-Null;if($LASTEXITCODE-ne 0){throw 'Isolated PostgreSQL container could not start.'};$created=$true
  ...
  docker exec $containerName createdb -U postgres phase10on
  Invoke-Sql @"
...
"@
  ...
  Invoke-SqlFile (Resolve-Path 'scripts/phase10o-n/permissions-smoke.sql').Path
  ...
  Invoke-SqlFile (Resolve-Path 'scripts/phase10o-n/concurrency-setup.sql').Path
  & powershell -ExecutionPolicy Bypass -File scripts/phase10o-n/run-concurrency.ps1 -ContainerName $containerName;if($LASTEXITCODE-ne 0){throw 'PHASE 10O-N concurrency acceptance failed.'}
  $tableCount=(docker exec $containerName psql -U postgres -d phase10on -tAc "SELECT count(*) FROM pg_catalog.pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='private' AND c.relkind='r'").Trim();if($tableCount-ne'8'){throw "Private table boundary mismatch: $tableCount"}
  Write-Output 'PHASE10O_N_ISOLATED_DB_OK private_tables=8 container_removed=true'
