...
const globals = window as unknown as { accountFixture: (people: boolean, deletion: boolean, emergency: boolean, kind: string) => void; fixture: (kind: string, writable: boolean, rows: SchoolMembership['class_history']) => void; refreshFixture: () => void; savedRows: SchoolMembership['class_history'] }
function Fixture() {
  const [kind, setKind] = useState('high')
  const [writable, setWritable] = useState(true)
  const [rows, setRows] = useState<SchoolMembership['class_history']>([])
  const [version, setVersion] = useState(0)
  const [account, setAccount] = useState<{ people: boolean; deletion: boolean; emergency: boolean } | null>(null)
  globals.accountFixture = (people, deletion, emergency, nextKind) => { setAccount({ people, deletion, emergency }); setKind(nextKind); setRows([]); setVersion(n => n + 1) }
  ...
  if (account) return <AccountClient key={version}
    state={{ adultEligible: true, consentsComplete: true, consentTypes: ['terms','privacy_collection','adult_confirmation','private_by_default'],
      deletionRequested: account.deletion, deletionStatus: account.deletion ? 'pending' : null,
      profile: { id: membership.id, owner_user_id: membership.id, display_name: 'Synthetic A', instagram_handle: null, introduction: null, profile_photo_url: null, profile_visibility: 'private', status: 'active', created_at: '', updated_at: '' }, memberships: [membership] }}
