...
const entry=process.argv[2], base=new URL(entry).origin, phase=process.argv[3]||'feature'
...
try{
 for(const [width,height]of [[390,844],[1280,900],[1440,900]]){
  ...
  await page.route('**/*',async r=>{
   ...
   if(request.method()==='POST'&&url.hostname==='hukokfyphyrpfouazxhq.supabase.co'&&['/rest/v1/rpc/search_schools_v2','/rest/v1/rpc/search_schools_with_activity'].includes(url.pathname)){
    const {q,lim}=request.postDataJSON();url.pathname='/rest/v1/rpc/search_schools_v2';url.search=new URLSearchParams({q,lim:String(lim)}).toString()
    const response=await page.request.get(url.toString(),{headers:request.headers()})
    blocked.push('READ_ONLY_GET_ADAPTER:'+request.url().split('/').at(-1))
    return r.fulfill({response})
   }
   if(!['GET','HEAD'].includes(request.method())){blocked.push(url.pathname);return r.abort()}return r.continue()
  })
  ...
  const loginMode=await page.getByRole('link',{name:'Google로 계속하기'}).count()?'google':'closed'
  if(loginMode==='google')await expect(page.getByRole('link',{name:'Google로 계속하기'})).toBeVisible()
  else await expect(page.getByRole('status')).toHaveText('로그인은 아직 열리지 않았습니다.')
  ...
  results.push({viewport:[width,height],home,school,loginMode,googleOnlyOrFailClosed:true,privateGuestRedirect:true,browserErrors:errors,blockedClientNonReadRequests:blocked})
