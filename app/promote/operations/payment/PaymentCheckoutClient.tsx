...
export default function PaymentCheckoutClient() {
  ...
  useEffect(() => { if(!sandboxPaymentEnabled) return setMessage('자동결제가 비활성 상태입니다.'); if(!paymentId) return; void fetch(`/api/payments?paymentId=${encodeURIComponent(paymentId)}`,{cache:'no-store'}).then(async r=>{ if(!r.ok) throw new Error(); const body=await r.json(); setPayment(body.payments?.[0]??null); setMessage('결제 전 주문번호와 금액을 확인하세요.') }).catch(()=>setMessage('결제 정보를 불러올 수 없습니다.')) },[paymentId])
  async function start() {
    if(!sandboxPaymentEnabled || !payment || payment.provider!=='portone_sandbox' || !callbackState) return setMessage('Sandbox 결제 설정이 필요합니다.')
    ...
    const result=await PortOne.requestPayment({
      ...
      customData:{ orderId:payment.order_id, orderNumber:payment.order_number },
    })
  ...
  return <main className="mx-auto max-w-xl px-5 py-12">{sandboxPaymentEnabled ? <Script src="https://cdn.portone.io/v2/browser-sdk.js" strategy="afterInteractive" /> : null}<h1 className="text-3xl font-bold">광고 결제</h1><p className="mt-3 text-sm text-gray-600">Sandbox 전용입니다. 실제 카드 결제와 Production webhook은 활성화되지 않았습니다.</p>{payment?<section className="mt-8 border p-5"><p className="font-bold">{payment.order_number}</p><p className="mt-2 text-2xl font-bold">{payment.amount_krw.toLocaleString('ko-KR')}원</p><p className="mt-2 text-sm text-gray-600">서버 저장 금액과 PG 조회 금액이 일치할 때만 결제 완료됩니다.</p><button onClick={()=>void start()} className="mt-5 min-h-12 w-full bg-gray-950 px-4 font-semibold text-white">Sandbox 결제 열기</button></section>:null}<p role="status" className="mt-5 border p-3 text-sm">{message}</p></main>
}
