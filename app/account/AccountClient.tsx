...
type Props={email:string;state:AccountState;launch:PublicAccountLaunch;currentYear:number}
...
export default function AccountClient({email,state,launch,currentYear}:Props){
  ...
  const accountWritable=(launch.privateProfileEnabled||launch.schoolMembershipEnabled)&&!deletionBlocked
  ...
  return <main className="mx-auto max-w-2xl px-5 py-10">
    ...
    {deletionBlocked ? <p className="mt-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-900" role="status">{state.deletionStatus==='pending'?'탈퇴 요청이 접수되어 추가 정보 변경을 차단했습니다.':'탈퇴 처리가 완료되어 개인 기능 접근을 차단했습니다.'}</p>:null}
    ...
    <section className="mt-8 rounded-2xl border border-gray-200 bg-white p-5"><h2 className="text-lg font-bold text-gray-950">1. 만 19세 이상 확인</h2>
      {state.adultEligible?<p className="mt-3 rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-800">현재 정책 기준 성인 확인 완료</p>:<form className="mt-4 space-y-3" onSubmit={async(event)=>{event.preventDefault();await submit('/api/account/eligibility',{dateOfBirth:birthDate})}}>
        ...
        <button disabled={busy||!accountWritable} className="schoollove-focus min-h-12 rounded-xl bg-gray-950 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">만 19세 이상 확인</button>
      </form>}
    </section>
    ...
    <section className="mt-5 rounded-2xl border border-gray-200 bg-white p-5"><h2 className="text-lg font-bold text-gray-950">2. 필수 동의</h2>
      {state.consentsComplete?<p className="mt-3 rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-800">현재 정책 버전의 필수 동의 완료</p>:<form className="mt-4 space-y-3" onSubmit={async(event)=>{event.preventDefault();await submit('/api/account/consents',consents)}}>
        ...
        <button disabled={busy||!accountWritable||!state.adultEligible} className="schoollove-focus min-h-12 rounded-xl bg-gray-950 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">필수 동의 4개 기록</button>
      </form>}
    </section>
    ...
    <section className="mt-5 rounded-2xl border border-gray-200 bg-white p-5"><h2 className="text-lg font-bold text-gray-950">3. 내 비공개 프로필</h2>
      ...
      <form className="mt-4 space-y-3" onSubmit={async(event)=>{event.preventDefault();await submit('/api/account/profile',{display_name:displayName,instagram_handle:instagram||null,introduction:introduction||null})}}>
        ...
        <button disabled={busy||!accountWritable||!state.adultEligible||!state.consentsComplete} className="schoollove-focus min-h-12 rounded-xl bg-gray-950 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">{state.profile?'내 프로필 수정 저장':'내 프로필 저장'}</button>
      </form>
    ...
    <section className="mt-5 rounded-2xl border border-gray-200 bg-white p-5"><h2 className="text-lg font-bold text-gray-950">4. 내 학교 이력 <span className="text-sm font-normal text-gray-500">({state.memberships.length}/3)</span></h2>
      ...
      <form className="mt-4 space-y-3" onSubmit={async(event)=>{event.preventDefault();if(!schoolId){setStatus('검색 결과에서 학교를 선택해 주세요.');setIsError(true);return}if(await submit('/api/account/memberships',{school_id:schoolId,graduation_year:Number(graduationYear),class_number:classNumber?Number(classNumber):null},'POST','학교 이력을 저장했습니다.')){setSchoolQuery('');setSchoolId('');setGraduationYear('');setClassNumber('')}}}>
        ...
        <button disabled={busy||!accountWritable||!state.profile||state.memberships.length>=3||!schoolId} className="schoollove-focus min-h-12 rounded-xl border border-gray-900 px-4 py-3 text-sm font-semibold text-gray-900 disabled:opacity-40">학교 이력 추가</button>
      </form>
    </section>

    <section className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-5"><h2 className="text-lg font-bold text-red-950">계정 탈퇴 요청</h2><p className="mt-2 text-sm leading-6 text-red-900">요청 즉시 프로필·학교 이력 변경을 차단합니다. 관리자가 확인 후 비공개 프로필과 학교 이력을 한 트랜잭션으로 삭제합니다. 성인 확인·필수 동의·감사 기록은 법적 증빙 정책에 따라 보존하며 Auth 계정은 재가입을 막는 장기 차단 tombstone으로 유지합니다.</p><p className="mt-2 text-xs text-red-800">영업일 1~3일 내 접수를 확인하고 원칙적으로 30일 이내 처리합니다. 접수 후 취소는 제공하지 않으며 오류 접수는 <Link href="/contact" className="underline">운영자 문의</Link>로 알려 주세요.</p><button type="button" disabled={busy||deletionBlocked} onClick={async()=>{if(window.confirm('탈퇴 요청 후에는 정보 변경과 요청 취소가 불가능합니다. 계속할까요?'))await submit('/api/account/deletion-request',{confirm:true},'POST','탈퇴 요청을 접수했습니다.')}} className="schoollove-focus mt-4 min-h-12 rounded-xl bg-red-800 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">{state.deletionStatus==='pending'?'탈퇴 요청 접수됨':state.deletionStatus==='done'?'탈퇴 처리 완료':'계정 탈퇴 요청'}</button></section>
