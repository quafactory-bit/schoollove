...
type NotificationItem = { id: string; type: 'request_received' | 'request_reminded' | 'request_accepted'; createdAt: string; read: boolean }
...
export default function ConnectionsClient() {
  ...
  const load = useCallback(async () => {
    const [requestsResponse, connectionsResponse, notificationsResponse] = await Promise.all([fetch('/api/connections/requests'), fetch('/api/connections'), fetch('/api/connections/notifications')])
    ...
    if (notificationsResponse.ok) { const data = await notificationsResponse.json(); setNotifications(data.notifications ?? []) }
  }, [])
