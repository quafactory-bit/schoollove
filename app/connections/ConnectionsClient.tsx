...
import { connectionStatusLabel, relationshipLabel } from '@/lib/connectionLabels'
...
export default function ConnectionsClient({ peopleSearchEnabled = false }: { peopleSearchEnabled?: boolean }) {
  ...
  return <main className="sl-game sl-connections mx-auto max-w-3xl px-5 py-10">
    ...
    <section id="received" className="mt-8"><h2 className="text-xl font-bold">받은 안부</h2><div className="mt-3 space-y-3">{received.length === 0 && <p className="rounded-xl bg-gray-50 p-4 text-sm text-gray-600">받은 안부가 없습니다.</p>}{received.map((item) => <article key={item.id} className="rounded-2xl border border-gray-200 bg-white p-5"><div className="flex justify-between gap-3"><p className="font-bold">{item.senderName}</p><span className="text-xs text-gray-500">{connectionStatusLabel(item.status)}</span></div>{item.school && <p className="mt-1 text-sm text-gray-600">{item.school.schoolName} · {item.school.graduationYear}년 · {relationshipLabel(item.relationshipType)}</p>}<p className="mt-4 whitespace-pre-wrap rounded-xl bg-gray-50 p-4 text-sm">{item.message}</p>{item.reminder && <p className="mt-2 text-xs font-semibold text-red-700">기존 안부가 한 번 더 알려졌습니다. 새 메시지가 아닙니다.</p>}{item.status === 'pending' && <div className="mt-4 flex flex-wrap gap-2"><button onClick={() => requestAction(item.id,'accept')} className="schoollove-dark-action rounded-lg bg-gray-950 px-3 py-2 text-sm text-white">안부 수락</button><button onClick={() => requestAction(item.id,'not_the_person')} className="rounded-lg border px-3 py-2 text-sm">아닌 것 같아요</button><button onClick={() => requestAction(item.id,'decline')} className="rounded-lg border px-3 py-2 text-sm">거절</button><button onClick={() => requestAction(item.id,'block')} className="rounded-lg border border-red-300 px-3 py-2 text-sm text-red-700">차단</button><button onClick={() => requestAction(item.id,'report','other')} className="rounded-lg border border-red-300 px-3 py-2 text-sm text-red-700">신고</button></div>}</article>)}</div></section>
    <section className="mt-8"><h2 className="text-xl font-bold">보낸 안부</h2><div className="mt-3 space-y-3">{sent.map((item) => <article key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-4"><div><p className="text-sm font-semibold">상태 · {connectionStatusLabel(item.status)}</p><p className="mt-1 text-xs text-gray-500">{new Date(item.sentAt).toLocaleString('ko-KR')}</p></div>{item.status === 'pending' && <div className="flex gap-2"><button onClick={() => remind(item.id)} disabled={item.reminderCount === 1} className="rounded-lg border px-3 py-2 text-xs disabled:opacity-40">안부 한 번 더 알리기</button><button onClick={() => cancel(item.id)} className="rounded-lg border border-red-300 px-3 py-2 text-xs text-red-700">취소</button></div>}</article>)}</div></section>
    <section id="connected" className="mt-8"><h2 className="text-xl font-bold">연결된 사람</h2>{connections.length > 0 ? <SceneImage scene="connections" className="sl-connections-art" /> : null}{connections.length === 0 ? <div className="sl-connections-empty"><SceneImage scene="connections" className="sl-scene--empty" /><p className="font-semibold">서로 안부를 수락하면 여기에 표시돼요.</p><p className="mt-2 text-sm text-gray-600">연결 후 상대가 허용한 인스타그램주소를 확인해요.</p></div> : null}<div className="mt-3 grid gap-3 sm:grid-cols-2">{connections.map((item) => <Link key={item.id} href={`/connections/${item.id}`} className="rounded-2xl border border-gray-200 bg-white p-5"><p className="font-bold">{item.displayName}</p><p className="mt-2 text-sm text-gray-600">{item.status === 'active' ? '연결 확인' : '종료된 연결'}</p></Link>)}</div></section>
    </> : null}
