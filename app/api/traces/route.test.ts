...
function requestThatMustNotBeRead(): NextRequest {
  return {
    json: async () => {
      throw new Error('legacy trace route read the request body')
    },
  } as unknown as NextRequest
}

describe('POST /api/traces — PHASE 10L permanent legacy-write boundary', () => {
  it('always returns the stable 503 boundary before parsing or persistence', async () => {
    ...
    expect(await response.json()).toEqual({
      ...
      code: 'LEGACY_TRACE_WRITE_PERMANENTLY_DISABLED',
    })
  })
})
