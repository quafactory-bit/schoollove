...
describe('PHASE 10O-P transaction-bound broker code migration', () => {
  ...
  it('closes the legacy service bypass and grants only bound issuance', () => {
    ...
    expect(source).toContain('GRANT EXECUTE ON FUNCTION public.get_transaction_bound_broker_code_issuance_context(uuid) TO service_role')
  })
  ...
  it('resolves restart-safe context only from a trusted attempt ID', () => {
    expect(source).toContain('CREATE FUNCTION public.get_transaction_bound_broker_code_issuance_context(target_attempt_id uuid)')
    expect(source).toContain("tx.status<>'upstream_bound'")
    expect(source).toContain("attempt.state NOT IN ('auth_principal_bound','existing_primary')")
    expect(source).toContain("leg.status<>'verified'")
    expect(source).toContain('RETURN QUERY SELECT tx.id,tx.login_attempt_id,tx.client_id,tx.redirect_uri,tx.pkce_s256_challenge,tx.downstream_nonce,tx.downstream_state,tx.expires_at')
  })
})
