...
describe('PHASE 10B auth/adult/ownership migration', () => {
  it('기존 profiles row를 삭제하거나 임의 owner에게 연결하지 않는다', () => {
    expect(sql).not.toMatch(/\b(?:DELETE|TRUNCATE)\s+(?:FROM\s+)?public\.profiles\b/i)
    expect(sql).not.toMatch(/UPDATE\s+public\.profiles\s+SET\s+owner_user_id/i)
    expect(sql).toContain("ADD COLUMN IF NOT EXISTS ownership_status text NOT NULL DEFAULT 'quarantined'")
