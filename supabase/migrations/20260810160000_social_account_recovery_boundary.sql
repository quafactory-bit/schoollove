...
CREATE UNIQUE INDEX auth_principal_cleanup_jobs_one_active_account
  ON private.auth_principal_cleanup_jobs(account_id)
  WHERE account_id IS NOT NULL AND status IN ('queued','retry_scheduled');
...
DECLARE verification_id uuid; account private.private_accounts%ROWTYPE;
BEGIN
  PERFORM private.require_social_service();
  SELECT * INTO account FROM private.private_accounts WHERE id=target_account_id FOR UPDATE;
  ...
  VALUES(target_account_id,'activation',requested_hmac,requested_hmac_key_version,requested_ciphertext,requested_nonce,requested_encryption_key_version,requested_otp_mac,requested_otp_key_version,clock_timestamp()+interval '10 minutes')
...
BEGIN
  ...
  ON CONFLICT (account_id) WHERE account_id IS NOT NULL AND status IN ('queued','retry_scheduled')
  DO UPDATE SET source_attempt_id=COALESCE(private.auth_principal_cleanup_jobs.source_attempt_id,EXCLUDED.source_attempt_id)
