...
describe('PHASE 10P expired unbound provisional resume migration', () => {
  ...
  it('preserves service-only authority and canonical lock ordering', () => {
    ...
    const postBrokerClock = sql.indexOf('now_at:=clock_timestamp();', brokerLock)
    const postBrokerCurrentCheck = sql.indexOf('IF attempt.id IS DISTINCT FROM target_attempt_id', postBrokerClock)
    ...
    const postSourceClock = sql.indexOf('now_at:=clock_timestamp();', identityLock)
    const postLockCurrentCheck = sql.indexOf('IF attempt.id IS DISTINCT FROM target_attempt_id', postSourceClock)
    const sourceTerminalization = sql.indexOf("UPDATE private.broker_authorization_codes SET state='expired'", postLockCurrentCheck)
    const targetAdoption = sql.indexOf("SET state='account_decided',broker_subject=requested_broker_subject", postLockCurrentCheck)
    expect(brokerLock).toBeGreaterThan(0)
    expect(postBrokerClock).toBeGreaterThan(brokerLock)
    expect(postBrokerCurrentCheck).toBeGreaterThan(postBrokerClock)
    expect(codeLock).toBeGreaterThan(postBrokerCurrentCheck)
    ...
    expect(postSourceClock).toBeGreaterThan(identityLock)
    expect(postLockCurrentCheck).toBeGreaterThan(postSourceClock)
    expect(sourceTerminalization).toBeGreaterThan(postLockCurrentCheck)
    expect(targetAdoption).toBeGreaterThan(postLockCurrentCheck)
