...
describe('PHASE 10P social activation and bound provisional reauth migration', () => {
  ...
  it('delegates legacy paths before candidate row locks and handles post-lock activation directly', () => {
    ...
    const postLockClock = wrapper.indexOf('now_at:=clock_timestamp();', identityLock)
    const postLockLiveness = wrapper.indexOf("tx.login_attempt_id IS DISTINCT FROM target_attempt_id", postLockClock)
    const expiredReturn = wrapper.indexOf("RETURN CASE WHEN next_tx_status='expired' THEN 'EXPIRED'", postLockLiveness)
    const boundReturn = wrapper.indexOf("RETURN 'BOUND_PROVISIONAL_REAUTH_READY'", postLockLiveness)
    const activeReturn = wrapper.indexOf("RETURN 'EXISTING_PRIMARY'", postLockLiveness)
    ...
    expect(postLockClock).toBeGreaterThan(identityLock)
    expect(postLockLiveness).toBeGreaterThan(postLockClock)
    expect(expiredReturn).toBeGreaterThan(postLockLiveness)
    expect(boundReturn).toBeGreaterThan(expiredReturn)
    expect(activeReturn).toBeGreaterThan(boundReturn)
