...
export const BetaAdminOperationSchema = z.discriminatedUnion('action', [
  z.object({ action: z.literal('issue_invite'), programId: z.string().uuid(), email: z.string().email().optional(), domain: z.string().min(3).max(255).optional(), maxUses: z.literal(1), expiresAt: z.string().datetime() }),
  ...
  z.object({ action: z.literal('set_feature'), programId: z.string().uuid(), userId: z.null(), feature: z.enum(betaFeatureKeys), enabled: z.boolean(), reason: z.string().regex(/^[A-Z0-9_]{2,60}$/) }),
