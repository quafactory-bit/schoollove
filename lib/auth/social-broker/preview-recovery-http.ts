...
function recoveryPage(stage: RecoveryContinuity['stage']): Response {
  if (stage === 'recovery_required') return new Response('<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="robots" content="noindex,nofollow,noarchive"><title>복구 이메일 확인</title><main><h1>복구 이메일 확인</h1><p>이 주소는 로그인 ID로 사용되지 않습니다.</p><p>소유권 확인과 중복 계정 보호를 위해 복구 이메일, 인증 결과와 시각을 처리합니다. 이메일은 암호문과 중복 확인용 해시로 보관하며, 암호문은 탈퇴의 개인 데이터 삭제 단계에서 제거합니다. 인증번호는 10분간 유효합니다.</p><p>인증번호 발송을 요청하면 복구 이메일과 인증번호가 담긴 본문을 미국의 Resend로 전송합니다. 인증번호의 유효기간과 발송 사업자의 메일·로그 보유기간은 다릅니다. <a href="/privacy#authentication">보유기간 안내</a>와 <a href="/privacy#processors">국외 이전·거부 방법</a>을 먼저 확인해 주세요. 복구 확인을 진행하지 않으면 개인 계정 시작을 완료할 수 없지만 공개 학교 정보는 이용할 수 있습니다.</p><form method="post"><input type="hidden" name="action" value="send"><label>복구 이메일 <input name="recovery_email" type="email" autocomplete="email" maxlength="254" required></label><button type="submit">인증번호 받기</button></form></main></html>', { status: 200, headers: { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' } })
