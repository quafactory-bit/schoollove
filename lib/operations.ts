...
export async function runMaintenance(runKey: string, asOf = new Date().toISOString()) {
  const admin = getSupabaseAdmin()
  const privacy = await admin.rpc('run_privacy_retention_cleanup')
  if (privacy.error || !privacy.data || privacy.data.ok !== true) throw new Error('PRIVACY_MAINTENANCE_FAILED')
  ...
  return { phase10f:phase10f.data, phase10h:phase10h.data, privacy:privacy.data }
}
