...
export async function buildOwnerExport(userId: string) {
  ...
  const [profile, memberships, classHistories, requests, connections, messages, connectionNotifications, consents, eligibility, promotionAccounts, promotionRequests, promotionOrders, promotionReports, payments, paymentRefunds, paymentDocuments] = await Promise.all([
    ...
    admin.from('notifications').select('kind,created_at,read_at').eq('user_id',userId).eq('in_app_visible',true).not('request_id','is',null).order('created_at',{ascending:false}),
  ...
  return {
    ...
    connectionNotifications: (connectionNotifications.data ?? []).map(({ kind, ...notification }) => ({ event_type: kind === 'connection_request' ? 'request_received' : kind === 'connection_reminder' ? 'request_reminded' : 'request_accepted', ...notification })),
