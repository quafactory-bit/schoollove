...
## A0. PHASE 10A 개인정보 안전 전환 — 최우선 계약

- 이 절은 아래의 기존 제품·FROZEN 계약과 충돌할 때 우선한다.
- 신규 개인 등록은 PHASE 10B 인증 경계가 완성될 때까지 서버에서 항상 차단한다.
- 향후 개인 등록은 만 19세 이상 이용자가 본인 정보만 등록하는 방식으로 제한한다.
...
- 사람 발견은 PHASE 10B의 성인 본인 인증·소유권·승인 경계가 완성된 뒤 재개한다.
- 페이지보다 필터를 우선한다.
- 등록보다 발견을 우선한다.
- 입력보다 기여를 우선한다.
...
- 기존 등록 모듈과 성공 상태는 PHASE 10B 재설계를 위해 보존할 수 있으나 공개 UI에서 호출하지 않는다.

## D. 공개 사용자 UX 계약

- PHASE 10B 전에는 일반 사용자 로그인이나 회원가입을 임의로 추가하지 않는다.
